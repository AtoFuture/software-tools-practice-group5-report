...
![Packet Tracer 拓扑结构](./截图/26-packettracer-topology.png)
...
![PC0 IP 配置](./截图/27-packettracer-pc0-ip.png)

PC1 配置：

![PC1 IP 配置](./截图/28-packettracer-pc1-ip.png)
...
![Packet Tracer Ping 成功](./截图/29-packettracer-ping-success.png)

### 2.5 Simulation 数据包观察

切换到 Simulation 模式后，使用 Add Simple PDU 观察 ICMP 数据包从 PC0 经交换机到 PC1 的转发过程。

![Packet Tracer Simulation 数据包流动](./截图/30-packettracer-simulation-pdu.png)
...
![Git 安装与版本验证](./截图/01-git-install-version.png)
...
![Node.js 与 npm 版本验证](./截图/02-node-npm-version.png)
...
![npm 镜像源配置](./截图/03-npm-registry.png)
...
![Hexo 版本验证](./截图/04-hexo-version.png)
...
![Hexo 本地预览](./截图/05-hexo-localhost-4000.png)
...
![VS Code 项目结构与报告页面](./截图/07-vscode-project-report-page.png)

### 4.2 Markdown 预览与报告撰写

使用 VS Code 编写 Markdown，并通过预览功能检查排版效果。

![VS Code Markdown 预览](./截图/08-vscode-markdown-preview.png)
...
![Git 初始化与首次提交](./截图/09-git-init-first-commit.png)
...
![Git 最终提交记录](./截图/24-git-final-log.png)
...
![Hexo 静态文件生成](./截图/06-hexo-generate-report.png)
...
![Ubuntu 安装时启用 OpenSSH](./截图/10-virtualbox-openssh-install.png)

### 6.3 安装完成

![Ubuntu Server 安装完成](./截图/12-ubuntu-install-complete.png)
...
![Ubuntu ip addr](./截图/13-ubuntu-ip-addr.png)
...
![Linux 用户与目录权限](./截图/19-linux-user-and-permission.png)
...
![SSH 服务状态](./截图/16-ssh-service-status.png)
...
![Nginx 服务状态](./截图/17-nginx-service-status.png)
...
![Linux 环境变量与 Nginx 配置验证](./截图/21-linux-nginx-config-and-env.png)
...
![VirtualBox NAT 端口转发](./截图/14-virtualbox-port-forwarding.png)
...
![Windows SSH 登录 Ubuntu](./截图/15-windows-ssh-login.png)
...
![SCP 上传 public 目录](./截图/18-scp-upload-public.png)
...
![Nginx 部署目录验证](./截图/20-nginx-deploy-directory.png)
...
![Nginx HTTP 200 OK 验证](./截图/22-http-nginx-200-ok.png)
...
![最终部署页面](./截图/23-final-report-page-8080.png)
